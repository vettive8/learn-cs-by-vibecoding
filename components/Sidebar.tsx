  ...
  return (
    <aside className="w-64 bg-[#121214] border-r border-zinc-800 hidden md:flex flex-col">
      <div className="p-6 border-b border-zinc-800">
        ...
        <p className="text-xs text-zinc-500 mt-1 uppercase tracking-widest font-semibold">Creative Mentor</p>
      </div>
      ...
      <nav className="flex-1 py-4 overflow-y-auto">
        ...
        {lessons.map((lesson) => (
          ...
          >
            ...
            <span className="text-sm font-medium truncate">{lesson.title.split(' ').slice(1).join(' ')}</span>
          </button>
        ))}
      </nav>
      ...
      <div className="p-6 border-t border-zinc-800">
        <div className="flex items-center gap-3 p-3 bg-blue-500/10 rounded-xl border border-blue-500/20">
          ...
          <p className="text-[10px] text-blue-300 uppercase tracking-tighter font-bold">Mentor Ready</p>
        </div>
      </div>
    </aside>
  );
