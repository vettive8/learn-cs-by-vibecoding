...
const VibecodingPrompt: React.FC<VibecodingPromptProps> = ({ topic, context }) => {
  ...
  const generatePrompt = async () => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: `Act as a creative coding mentor. I want to 'vibecode' a project about: ${topic}.
        Context: ${context}
      ...
      console.error(error);
      setPrompt("Error connecting to Gemini. Please try again.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="mt-12 bg-zinc-900 border border-zinc-800 rounded-3xl overflow-hidden">
      <div className="p-6 border-b border-zinc-800 flex items-center justify-between">
        <div>
          <h4 className="text-lg font-bold flex items-center gap-2">
            ...
            AI Studio Vibecoding Prompt
          </h4>
          <p className="text-xs text-zinc-500 font-mono">Generate a prompt for Google AI Studio</p>
        </div>
        <button 
        ...
        >
          {loading ? 'Thinking...' : 'Generate Magic Prompt'}
        </button>
      </div>
      ...
      {prompt && (
        <div className="p-6 space-y-4 animate-in slide-in-from-bottom-2 duration-300">
          ...
          <p className="text-center text-[10px] text-zinc-600 uppercase tracking-widest font-bold">Paste this into Google AI Studio to start your vibecoding session</p>
        </div>
      )}
    </div>
  );
